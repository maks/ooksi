...
//    + " 3t(1-t)^2 "+fpToString(DT1MT2[i]));
  ...
  public void evalNative(int id, JsArray stack, int sp, int parCount) {
    switch (id){
      ...
      case ID_DRAW_IMAGE:
      	if (stack.isNull(sp+2) || stack.isNumber(sp+2)) {
      		//FIXME: should we throw error instead ??
      		return;
      	}
    	ImageObject imgObj = (ImageObject)stack.getObject(sp + 2);
    	
    	if (stack.isNumber(sp+3) && stack.isNumber(sp+4)) {
    		if ( stack.isNull(sp+5)) {
    			//call for drawImage(img, x, y)
    			int sx = (int)stack.getNumber(sp + 3);
    	        int sy = (int)stack.getNumber(sp + 4);
    	        imgObj.drawToMidpCanvas(graphics, sx, sy);
        		
    		} else if (stack.isNumber(sp+5) && stack.isNumber(sp+6)) {
    			if (stack.isNull(sp+7)) { 
    				//call for drawImage(img, x, y, width, height)
